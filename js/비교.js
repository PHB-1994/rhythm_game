$(function () {
  ...
  let gameInterval; // 떨어지는 아이템 생성 후 id 값을 설정
  let timerInterval; // 타이머 ID

  // 키보드와 키도으에 해당하는 번호를 연결 설정
  const keyMap = {
    d: 0,
    f: 1,
    j: 2,
   ...
   */
  function endGame() {
    gameActive = false; // 게임 상태를 비활성화 처리
  ...
  function 성공함수(laneIndex) {
    ...
    const effect = $("<div class='hit-effect'>").css({
      // 레인 중앙에 위치하도록 x 좌표 계산(효과 크기의 절반만 보정)
      left: laneOffset.left + lane.width() / 2 - 30 + "px",
      // 판정선 위쪽에 위치하도록 y 좌표 설정
      top: $("#game-container").height() - 80 + "px",
    });
  ...
  $(document).keydown(function (e) {
    ...
    // 객체에서.hasOwnProperty(사용자가작성한키보드입력값을)
    //          가지고 있나요?
    if (!keyMap.hasOwnProperty(key)) {
      // 가지고 있지 않은게 사실이라면!
      // 아래 코드를 수행하지 못하게 돌려보내기
      return;
    }
    ...
    $(".note").each(function () {
      ...
      if ($(this).data("lane") === lane) {
        ...
        if (Math.abs(notePos - judgeLine) < 50) {
          ...
          // setTimeout 을 이용해서 입력한 키보드 효과를 0.3 초 후 누름 뗌
          // CSS 제공
